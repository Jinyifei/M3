...
c
      ...
      real*8 tmp7,tmp8,tmp9,tmp10,tmp11,tmp12
      real*8 scal
      real*8 engout(22)
c
...
c
      real*8 kabp,ksct      
c
      integer*4 lulin,luop1,luop2,luop3,luop4,luop5
c
      character*512 outfile1, outfile2, outfile3
c
      character*512 outfile4, outfile5, outfile6
c
      character*512 starfile, outfile
c      
      logical iexi      
c
...
c      luop5=4
c
    5 format(1pe11.4,' ',1pe11.4,' ',1pe11.4,' ',1pe11.4,' ',
     &       1pe11.4,' ',1pe11.4,' ',1pe11.4,' ',1pe11.4,' ',
     &       1pe11.4,' ',1pe11.4)
c
      lulin=7
      luop2=9
...
c
c
c      open (lulin,file=trim(outfile4),
c     &    status='unknown')
c          do i = 1, mcnx+1
c           write (lulin,*) cwx(i), cwy(i), cwz(i)
c          enddo
c      close (lulin)
c
c
...
c
         if ((tei.le.0.0d0).or.(dei.le.0.0d0)) then
c
            do inl=1,infph-1
               jnu_arr(ix,iy,iz,inl) = 0.0d0
               recpdf_arr(ix,iy,iz,inl) = 0.0d0
               sigmt_arr(ix,iy,iz,inl,1) = 0.0d0
            enddo
c
            goto 15
c            
...
c         
         do inl=1,infph-1
            ...
            sigmt_arr(ix,iy,iz,inl,1)=sigmt*dhni
c
            if (grainmode.eq.1) then
               call dcrosint(inl,kabp,ksct)
               sigmt_arr(ix,iy,iz,inl,2)=kabp*1.0d0/sigmt
            endif
c
         enddo 
c
...
c
      if (taskid.eq.63) then
      open (lulin,file='scat.out',status='unknown')
      do inl=1,infph-1
         write (lulin,*),photev(inl),sigmt_arr(13,13,13,inl,2)
     &                  ,sigmt_arr(62,62,62,inl,2)   
      enddo
      close(lulin)
      endif
c      
c      if (m.eq.1) then
c         open (lulin,file=trim(outfile2),
c     &    status='unknown')
c
c          do inl=1,infph-1
c           write (lulin,*) photev(inl)
c     &     , soupho(inl)
c          enddo
c          close (lulin)
c          stop
...
c
     ...
     &     ,jnu_arr(11,12,15,inl), jnu_arr(13,12,15,inl)
     &     ,jnu_arr(2,2,3,inl), jnu_arr(2,2,7,inl)
     &     ,recpdf_arr(1,2,5,inl),recpdf_arr(3,2,5,inl)
     &     ,recpdf_arr(2,2,3,inl),recpdf_arr(2,2,7,inl)
     &     ,sigmt_arr(1,2,5,inl,1),sigmt_arr(3,2,5,inl,1)
     &     ,sigmt_arr(2,2,3,inl,1),sigmt_arr(2,2,7,inl,1)
...
c
         intpt = inttphot
c
         if ((inttphot.le.epsilon).or.(dhni.le.epsilon)) goto 25                
c
...
c        Calculate Tau
         ...
         do inl=1,infph-1
            ...
            dustsigmat=0.d0
            call crosssections (inl, tauso, sigmt, dustsigmat)
            sigmt_arr(ix,iy,iz,inl,1)=sigmt*dhnf
            sigmtso=sigmtso+sigmt*tphot(inl)/inttphot
c
            if (grainmode.eq.1) then
               call dcrosint(inl,kabp,ksct)
               sigmt_arr(ix,iy,iz,inl,2)=kabp*1.d0/sigmt
            endif            
         enddo 
c         
...
c
     ...
     &                      convg_arr(ix,iy,iz),
     &                      q1,q2,q3,q4,
     &                      hden_arr(ix,iy,iz),inttphot
c
c            
c
            write (luop3,5) hbeta,f5007,f6584,f6720
     &                     ,f3727,f4363,f6300,halpha
     &                     ,caseab(1),hydrobri(2,2)
cccc--------------------
ccc            write (luop3,5) halpha,hbeta
ccc     &                     ,hydrobri(1,2),hydrobri(2,2)
ccc     &                     ,def,tef,dhnf,zion(1)
ccc     &                     ,pop(2,1),caseab(1)
cccc--------------------
c
c
...
c  Output Spectrum
   35 format(//' ======================================'//
     ...
     &         '=============================')
c
      open (lulin,file=trim(outfile2),status='unknown') 
      write (lulin,35)
c
...
c
         nstar = 1
c         
         strpos_arr(1,1) = 0.0d0
         strpos_arr(2,1) = 0.0d0
         strpos_arr(3,1) = 0.0d0
         strpos_arr(1,2) = 0.0d0
         strpos_arr(2,2) = 0.0d0
         strpos_arr(3,2) = 0.0d0
c         
c
         iexi=.false.
         inquire (file=starfile,exist=iexi)
         if (iexi.eqv..True.) then
c
            tmp1 = 0
c
            open (luop4,file=trim(starfile),status='OLD')
            read (luop4,*) tmp1
c            
            do while (nstar.le.tmp1)
               read (luop4,*) tmp2,tmp3,tmp4,scal
               strpos_arr(1,1) = tmp2
               strpos_arr(2,1) = tmp3
               strpos_arr(3,1) = tmp4
               nstar = nstar + 1
c             
c               icheck = 1
               call mcrt0 (m,incream,blum,blum0,1,scal)               
c 
            end do
c
            close (luop4)
...
c         
            ...
            call mcrt0 (m,incream,blum,blum0,1,1.0d0)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
      subroutine mcrt0 (m,incream,blum,blum0,numstar,scal)
c
      include 'cblocks.inc'
      include 'mpif.h'
c    
...
c
cccccccccccccc    
       strpos(1) = strpos_arr(1,numstar)
       strpos(2) = strpos_arr(2,numstar)
       strpos(3) = strpos_arr(3,numstar)
c      strpos(1) = 0.0d0
c      strpos(2) = 0.0d0
c      strpos(3) = 0.0d0
...
c            mcde=soupho(nup)*wid*evplk*invnph*blum*invblum
            ...
            mcde=0.0d0
            mcde=soupho(nupcnt)*wid*evplk*invnph*blum*invblum*scal               
c                        
c
cc            if((taskid.eq.0).and.(icheck.eq.1)
cc     &     .and.(soupho(nupcnt).gt.0.d0)) then 
cc              write (*,*) mcde,scal,blum,blum0,
cc     &                    soupho(nupcnt),invnph,invblum
cc              icheck=0
cc            endif
c
c            
...
c              Calculate local Tau
               sigmt=0.d0 
               tauloc=0.d0      
               sigmt=sigmt_arr(ci(1),ci(2),ci(3),nupdp,1)       
               if (sigmt.gt.0)
     &         tauloc=sigmt*ds
c
c               if (sigmt.gt.0)
...
c                  
                  random=ran2(seed)
                  if ((random.le.sigmt_arr(ci(1),ci(2),ci(3),nupdp,2))
     &           .and.(grainmode.eq.1)) then 
c                 absorbed by Dust and re-emitted as IR flux                  
                     lgescape=.true.
                     lgfin = .true.
                     goto 50                    
                  else                 
...
c
                  if (isnan(jnu_arr(ci(1),ci(2),ci(3),nupdp)))
     &            then
                     write (*,*) 'noabs',nupdp,mcde,ds,invblum
     &                           ,ci(1),ci(2),ci(3)
     &                           ,ds*mcde/dv,dv,taskid
                     stop
                  endif
...
c     stellar photon      
c      if (lgstr.and.(.not.lgdif)
c     &   .and.(random.ge.strpdf_arr(numstar,nup))) goto 30
c     diffuse photon      
