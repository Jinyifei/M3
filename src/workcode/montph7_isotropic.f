...
ccccc      do iy = 1, mcny
ccccc      do iz = 1, mcnz  
ccc         rtmp = dsqrt(((cwx(ix)+cwx(ix+1))*0.5d0)**2.d0
ccc     &               +((cwy(iy)+cwy(iy+1))*0.5d0)**2.d0
ccc     &               +((cwz(iz)+cwz(iz+1))*0.5d0)**2.d0)
c
c
ccc         if ((rtmp .gt. 1.39d19).and.(rtmp .lt. 1.41d19)) then
ccc            nsum1=nsum1+1
ccc            do inl=1,infph-1
ccc               jnuw_arr(1,inl) = jnuw_arr(1,inl)+jnu_arr(ix,iy,iz,inl) 
ccc            enddo
ccc         endif
ccc         if (dabs(rtmp-1.3d19).le.0.5d18) then
ccc            nsum2=nsum2+1
ccc            do inl=1,infph-1    
ccc               jnuw_arr(2,inl) = jnuw_arr(2,inl)+jnu_arr(ix,iy,iz,inl)      
ccc            enddo
ccc         endif   
ccccc      enddo
ccccc      enddo
...
c    
c       
ccc      open (lulin,file='./output/structure_jspec',status='unknown') 
c
ccc      do inl=1,infph-1
ccc         write (lulin,*) photev(inl),soupho(inl)
ccc     &               ,jnuw_arr(1,inl), jnuw_arr(2,inl) 
ccc     &               ,jnu_arr(2,13,13,inl), jnu_arr(7,13,13,inl) 
ccc      enddo
ccc      close(lulin) 
c
ccc      do inl=1,infph-1
ccc         jnuw_arr(1,inl)=0.d0        
ccc         jnuw_arr(2,inl)=0.d0        
ccc      enddo
c
c
     ...
     &                 REC0, DLOS, ELOSS, EGAIN'     
      close(lulin)
c
c      open (luop1,file='./output/emission.out',status='unknown')
c      write (luop1,*) 'x, y, z, 
c     &                H-beta,f5007,f4363,f3727,f6300,f6584,f6720'
c      close(luop1)
c
      do ix = 1, mcnx
      do iy = 1, mcny
...
c
c
c         open (luop1,file='./output/emission.out',
c     &        status='OLD',access='APPEND') 
c     
c         write (luop1,*) (cwx(ix)+cwx(ix+1))*0.5d0,
