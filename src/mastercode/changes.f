...
c
   ...
   52   format(/' File name to save: ',$)
        if (taskid.eq.0) read (*,53,err=51) fnam
      call mpi_barrier(MPI_COMM_WORLD, taskerr)
      call mpi_bcast(fnam,512,MPI_CHARACTER,0,MPI_COMM_WORLD,taskerr)
c
   53   format(a)
...
c
   ...
   70    format(/' Enter file name : ',$)
         if (taskid.eq.0) read (*,80,err=60) fnam
      call mpi_barrier(MPI_COMM_WORLD, taskerr)
      call mpi_bcast(fnam,512,MPI_CHARACTER,0,MPI_COMM_WORLD,taskerr)
   80    format(a)
        m=lenv(fnam)
...
c
  ...
  220 format(' ::::::::::    Alter?(y/n) : ',$)
      if (taskid.eq.0) read (*,230,err=210) ilgg
  230 format(a)
      call mpi_barrier(MPI_COMM_WORLD, taskerr)
      call mpi_bcast(ilgg,4,MPI_CHARACTER,0,MPI_COMM_WORLD,taskerr)
c
...
c
  280 format(a)
          if (taskid.eq.0) read (*,280) blanc
      call mpi_barrier(MPI_COMM_WORLD, taskerr)
      call mpi_bcast(blanc,80,MPI_CHARACTER,0,MPI_COMM_WORLD,taskerr)
c
          ilgg=blanc(1:1)
          if (ilgg.eq.'x') ilgg='X'
          if (ilgg.eq.'c') ilgg='C'
