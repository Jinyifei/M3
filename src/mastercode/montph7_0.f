...
c
         if ((difg.le.1).and.(pop(1,1).ge.fren)) cov=cov+1.d0
         if (pop(1,1).ge.fren) totion=totion+1.d0
...
c      
   40 m=m+1
c
      if (m.eq.1) open (lulin,file='./output/status.out', 
     &        status='unknown')
      if (m.gt.1) open (lulin,file='./output/status.out',
     &        status='OLD',access='APPEND')
...
c         npck = 1.d5
c
c         call mcrt0 (m,incream,blum,blum0)   
         call mcrt0 (m,blum,blum0)   
c
         goto 20  
c
      endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
      subroutine mcrt0 (m,blum,blum0)
c      subroutine mcrt0 (m,incream,blum,blum0)
c
      include 'cblocks.inc'
      include 'mpif.h'
c    
...
c
      real*8    sigmt 
c
      real*8    enpck(looplmt,infph,6)
      integer*4 nuparr(looplmt,infph)
      logical   flg1(looplmt,infph), flg2(looplmt,infph)
c
c
cccccccccccccc    
      integer*4 lulin      
...
c
c      npck0=npck*infph
c      npck0=npck*incream
c
c      invnph=1.d0/nphot
      invnph=1.d0/npck
      invblum=1.d0/blum0
c      
      if (npck.gt.looplmt) then
         nphtlop = npck/looplmt
         if (nphtlop*looplmt.lt.npck) nphtlop=nphtlop+1
      else
         nphtlop = 1
      endif 
c
...
c
c
c      write (*,*) nphot, npck0, npck, infph
      write (*,*) nphot, npck, infph     
c
...
c      do nup0 = 1, infph-1
      do ilop = 1, nphtlop
c
         if (ilop.eq.nphtlop) then
             nphot = npck-(ilop-1)*looplmt
         else 
             nphot = looplmt
         endif
c
c
         do nup_init = 1, infph-1 
         do iphot = 1, nphot             
c
            nup0 = nup_arr(iphot,nup_init)
c
