...
c
      ...
      enddo  
      do i=2,mcny+1
         cwy(i) = cwy(i-1) + 0.25d18
      enddo      
      do i=2,mcnz+1
         cwz(i) = cwz(i-1) + 0.25d18
      enddo
c
      do i=1,mcnx+1
...
c
     ...
     &               jnu_arr(9,2,2,inl),jnu_arr(14,2,2,inl)
c     &               jnuw_arr(1,inl), jnuw_arr(2,inl)    
      enddo
      close(lulin) 
c
      do inl=1,infph-1
...
c         call teequi2 (tei, tef, def, dhni, 1.d33, nmod)
c
         dhnf=dhni
c
c        --------------------
...
c        Calculate Tau
         ...
         enddo 
c         
         write (lulin,*) ix,iy,iz,tef,def,dhni,q1,q4
c
...
c                 Initialise photon Direction                
                  ...
                  hvec(3)=0.0d0         
ccc                  if (lgdif) then
ccc                     call photdir (hvec)
ccc                     if (hvec(1).gt.0) then
ccc                        hvec(1)=1.0d0
ccc                        hvec(2)=0.0d0
ccc                        hvec(3)=0.0d0
ccc                     endif
ccc                     if (hvec(1).lt.0) then
ccc                        hvec(1)=-1.0d0
ccc                        hvec(2)=0.0d0
ccc                        hvec(3)=0.0d0
ccc                     endif                     
...
c
               if (abs(dsqrt((rvec(1)-strpos(1))**2.0d0
     &                      +(rvec(2)-strpos(2))**2.0d0
     &                      +(rvec(3)-strpos(3))**2.0d0)-0.75d18)
     &                       .le.0.00001d18)
     &         jnuw_arr(1,nup) = jnuw_arr(1,nup)+ds*mcde/dv 
               if (abs(dsqrt((rvec(1)-strpos(1))**2.0d0
     &                      +(rvec(2)-strpos(2))**2.0d0
     &                      +(rvec(3)-strpos(3))**2.0d0)-1.75d18)
     &                       .le.0.00001d18)               
     &         jnuw_arr(2,nup) = jnuw_arr(2,nup)+ds*mcde/dv               
c
