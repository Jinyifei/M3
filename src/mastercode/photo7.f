...
c
  ...
  290 format(/'  Choose a Pressure Structure '/
     ...
     &     ' :: ',$)
        read (*,90) jpres
        jpres=jpres(1:1)
        if ((jpres.eq.'a').or.(jpres.eq.'A')) jpres='A'
        if ((jpres.eq.'f').or.(jpres.eq.'F')) jpres='F'
...
c
        if (jpres.eq.'F') then
          rp0=0.d0!settoremptywhenu0solvedforponk0+ponk1
...
c
     ...
     &     .and.(ilgg.ne.'u').and.(ilgg.ne.'n')) goto 400
c
          if (ilgg.eq.'u') then
            write (*,420)
  420       format(//,' Give U(H) at inner radius (<=0 as log) : ',$)
            read (*,*) uinit
c
...
c
          if (ilgg.eq.'n') then
            write (*,430)
  430       format(//,' Give U(N) at inner radius (<=0 as log) : ',$)
            read (*,*) uinit
c
            if (uinit.le.0) uinit=10.d0**uinit
c
...
c
          if (ilgg.eq.'q') then
            write (*,440)
  440       format(//,' Give QHDN at inner radius (<= 100 as log) : ',$)
            read (*,*) qinit
c
...
c
          if (ilgg.eq.'h') then
            write (*,450)
  450       format(//,' Give QHDH at inner radius (<= 100 as log) : ',$)
            read (*,*) qinit
c
            if (qinit.le.100) qinit=10.d0**qinit
c
...
c
            write (*,460)
  460       format(//' Give the initial radius ',
...
c
  510     write (*,520)
  520    format(/,' Give Ionizing Flux at inner edge by: ',/,
     &     ' ::::::::::::::::::::::::::::::::::::::::::::::::::::',/,
...
c
          ...
          if (ilgg.eq.'B') then
            write (*,530)
  530       format(/,'Give Bolometric flux at inner edge of cloud:')
            read (*,*) scale
            scale=scale/blum
          elseif (ilgg.eq.'I') then
            write (*,540)
  540       format(/,'Give Ionising flux at inner edge of cloud:')
            read (*,*) scale
            scale=scale/ilum
  ...
  550       format(/,
     &      'Give Ionising Photon flux at inner edge (<=100 as log):')
          ...
          elseif (ilgg.eq.'U') then
            write (*,560)
  560     format(/,'Give U(H) at inner edge (<=0 as log):')
            read (*,*) scale
            if (scale.le.0.d0) scale=10**scale
            scale=scale*dhn*cls/qht
          elseif (ilgg.eq.'N') then
            write (*,570)
  570     format(/,'Give U(N) at inner edge (<=0 as log):')
            read (*,*) scale
            if (scale.le.0.d0) scale=10**scale
            scale=scale*dht*cls/qht
          elseif (ilgg.eq.'Q') then
            write (*,580)
  580       format(/,'Give QHDN at inner edge (<=100 as log):')
            read (*,*) scale
            if (scale.le.100.d0) scale=10**scale
            scale=scale*dht/qht
          elseif (ilgg.eq.'H') then
            write (*,590)
  590       format(/,'Give QHDH at inner edge (<=100 as log):')
            read (*,*) scale
            if (scale.le.100.d0) scale=10**scale
            scale=scale*dhn/qht
...
c
  610     write (*,620)
  620    format(/' Give the geometrical dilution factor (<=0.5) : ',$)
          read (*,*) wdpl
c
...
c
  ...
  730       format(/'  Source turn-off and final luminosity'/
     &       ' ::::::::::::::::::::::::::::::::::::::::::::::::::::'/
...
c
  740 write (*,750)
  750 format(//' Give the step photon absorption fraction: ',$)
      read (*,*) dtau0
...
c
      if (expertmode.gt.0) then
  760    format(/' Do you wish to alter the convergence criteria ?',
     &        ' (not recommended) : ',$)
        write (*,760)
        read (*,90) ilgg
        ilgg=ilgg(1:1)
...
c
  ...
  800 format(//' Model Summary :',/
     ...
     & ,'    dTau        :',0pf8.5)
      ...
      if (usekappa) then
  810    format(/' Kappa Electron Distribution Enabled :'/
     &           '    Electron Kappa : ',1pg11.4)
        write (*,810) kappa
      endif
c
  820 format(/' Radiation Field and Parameters:',//,
...
c
  840 format(//' ::::::::::::::::::::::::::::::::::::::::::::::::::::'/
     ...
     & ' ::::::::::::::::::::::::::::::::::::::::::::::::::::')
      write (*,840)
c
  850 format(/'  Choose a model ending : '/
     &     ' ::::::::::::::::::::::::::::::::::::::::::::::::::::'/
...
c
     ...
     &.and.(jend.ne.'R').and.(jend.ne.'G')) goto 830
      if (jend.eq.'R') return
...
c
c
 1020 format(//'  Choose output settings : '/
     & ' :::::::::::::::::::::::::::::::::::::::::::::::::::::'/
...
c
      if (jiel.eq.'YES') then
 1030    format(//' Give 4 elements to monitor (atomic numbers): ',$)
        write (*,1030)
c
...
c
      if (jiem.eq.'YES') then
c
 1040 format(//' Choose 4 multi-level species #',i2,' :',/
     & '::::::::::::::::::::::::::::::::::::::::::::::::::::'/)
        write (*,1040) ,iem
        do i=1,(nfmions/5)+1
          j=(i-1)*5
...
c
      if (jlin.eq.'YES') then
c
 1200  format(//' Choose up to ',i2,' lines by wavelength (A)   :',/
     & '::::::::::::::::::::::::::::::::::::::::::::::::::::'/)
 1210  format(' Number of lines : ',$)
 1220  format(/' #',i2,' : ',$)
        write (*,1200) mxmonlines
        write (*,1210)
        read (*,*) nl
...
c
 1060   format (a16)
 1070   format(//' Give a prefix for final source file : ',$)
        write (*,1070)
        read (*,1060) jpfx
        nprefix=1
 1061 if ((jpfx(nprefix+1:nprefix+1).eq.' ')
     &     .or.(nprefix.ge.8)) goto 1062
          nprefix=nprefix+1
        goto 1061
 1062   continue
        jpfx=jpfx(1:nprefix)
...
c     get final field file prefix
c
 1080   format(//' Give a prefix for first ion balance file : ',$)
        write (*,1080)
        read (*,1060) jbfx
        nprefix=1
 1081   if ((jbfx(nprefix+1:nprefix+1).eq.' ')
     &     .or.(nprefix.ge.8)) goto 1082
          nprefix=nprefix+1
          goto 1081
 1082   continue
        jbfx=jbfx(1:nprefix)
...
c
 1090 format (a96)
 1100 format(//' Give a name/code for this run: ',$)
      write (*,1100)
      read (*,1090) runname
c
...
c
      call p7fheader (newfil, banfil, fnam, filna, filnb, filnc, filnd,
...
c
ccc      if (m.eq.1) then
ccc      open (luop,file='specpp1')
ccc          do i=1,infph-1
ccc            write(luop,*) photev(i), tphot(i), sphot(i), soupho(i)
ccc          enddo      
ccc      close(luop)
ccc      endif
c
      if (dabs(disav-7.80715e+18).le.1.0d15) then
         open (luop,file='./output/specpp1')
             do i=1,infph-1
               write(luop,*) photev(i), tphot(i), sphot(i), soupho(i)
             enddo      
         close(luop)         
      endif      
c
      if (dabs(disav-1.43010e+19).le.1.0d15) then
         open (luop,file='./output/specpp2')
             do i=1,infph-1
               write(luop,*) photev(i), tphot(i), sphot(i), soupho(i)
             enddo      
         close(luop) 
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
ccc      open (luop,file='specpp2')
ccc          do i=1,infph-1
ccc            write(luop,*) photev(i), tphot(i), sphot(i), soupho(i), 
ccc     &                    resflux(i), resflux2(i),
ccc     &                    popint(1,1), popint(2,1)
ccc          enddo      
ccc      close(luop)
c
      open (luop,file=filna,status='OLD',access='APPEND')
