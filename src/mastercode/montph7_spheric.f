...
ccc  100 format(/' Input Density File : ',$)
ccc      read (*,20) fnam

ccc      m=lenv(fnam)
ccc      denfile=fnam(1:m)
c
c
...
c
      ...
      do iy = 1, mcny
      do iz = 1, mcnz  
         rtmp = dsqrt(((cwx(ix)+cwx(ix+1))*0.5d0)**2.d0
     &               +((cwy(iy)+cwy(iy+1))*0.5d0)**2.d0
     &               +((cwz(iz)+cwz(iz+1))*0.5d0)**2.d0)
c
         if (dabs(rtmp-1.0d19).le.0.5d18) then
            nsum1=nsum1+1
            do inl=1,infph-1
               jnuw_arr(1,inl) = jnuw_arr(1,inl)+jnu_arr(ix,iy,iz,inl) 
            enddo
         endif
         if (dabs(rtmp-1.3d19).le.0.5d18) then
            nsum2=nsum2+1
            do inl=1,infph-1    
               jnuw_arr(2,inl) = jnuw_arr(2,inl)+jnu_arr(ix,iy,iz,inl)      
            enddo
         endif   
      enddo
      enddo
...
c           
      open (lulin,file=
     &     '/Users/jyf/Desktop/code_check/element/structure_jspec'
     &     ,status='unknown') 
c
      do inl=1,infph-1
         write (lulin,*) photev(inl),soupho(inl),             
     &               jnuw_arr(1,inl)/nsum1, jnuw_arr(2,inl)/nsum2  
      enddo
      close(lulin) 
c
      do inl=1,infph-1
...
c
      open (lulin,file=
     &     '/Users/jyf/Desktop/code_check/element/structure'
     &     ,status='unknown') 
c            
      do ix = 1, mcnx
      do iy = 1, mcny
...
c         write (*,*) ix,iy,iz
         call mcteequi (tei, tef, def, dhni, nmod)      
c
         dhnf=dhni
c
c        --------------------
...
c         
     ...
     &                   tef,def,q1,q4,
c
ccc         write (lulin,*) ix,iy,iz,tef,def,dhni,q1,q4,
ccc     &         volfilar(ix,iy,iz),
     &         pop(1,1),pop(2,1),
     &         pop(1,2),pop(2,2),pop(3,2), 
c     &         pop(1,6),pop(2,6),pop(3,6),pop(4,6),pop(5,6),
c     &         pop(1,7),pop(2,7),pop(3,7),pop(4,7),pop(5,7),
     &         pop(1,8),pop(2,8),pop(3,8),pop(4,8),pop(5,8),
     &         pop(6,8),pop(7,8),pop(8,8),pop(9,8),
     &         pop(1,16),pop(2,16),pop(3,16),pop(4,16),pop(5,16),
     &         pop(6,16),pop(7,16),pop(8,16),pop(9,16)
ccc     &         dlos,hloss,rloss,fslos,fmloss,xr3loss,xrlloss, 
ccc     &         fflos,colos,f3loss,feloss,gcool,   
ccc     &         pgain,cosgain,paheat,gheat,chgain,rngain                  
c         
   30 continue 
      enddo
      enddo 
...
cccccccccccccc    
c      
c     Calculate volume filling factor per cell  
c      open (lulin,file='/Users/jyf/Desktop/code_check/fill'
c     &     ,status='unknown') 
ccc      if ((m.eq.1).and.(diend.ne.-1.d0)) then   
ccc         do ix=1, mcnx
ccc         do iy=1, mcny
ccc         do iz=1, mcnz
ccc            volfilar(ix,iy,iz)=0.d0      
ccc            rx0=cwx(ix)
ccc            ry0=cwy(iy)
ccc            rz0=cwz(iz)
ccc            do icont=1,10000
ccc               seed=86432
ccc               random=ran2(seed)                 
ccc               rx=rx0+mcdx*random
ccc               random=ran2(seed)                 
ccc               ry=ry0+mcdy*random
ccc               random=ran2(seed)                 
ccc               rz=rz0+mcdz*random
ccc               if (((rx**2+ry**2+rz**2).le.(diend**2.d0)).and.
ccc     &             ((rx**2+ry**2+rz**2).ge.(remp**2.d0)))
cc               if ((rx**2+ry**2+rz**2).le.(diend**2.d0))
ccc     &            volfilar(ix,iy,iz)=volfilar(ix,iy,iz)+1.d0/10000.d0
c               if ((ix.eq.1).and.(iy.eq.10).and.(iz.eq.16))
c     &            write (lulin,*) rx,ry,rz,volfilar(ix,iy,iz)              
ccc            enddo
c            write (lulin,*) ix,iy,iz,rx0,ry0,rz0
c     &       ,volfilar(ix,iy,iz)  
ccc         enddo
ccc         enddo
ccc         enddo     
...
c      stop      
c
cc      if ((m.eq.1).and.(diend.eq.-1.d0)) then   
         do ix=1, mcnx
         do iy=1, mcny
         do iz=1, mcnz
            volfilar(ix,iy,iz)=1.d0
         enddo
         enddo
         enddo     
cc      endif
                  ...
                  if (dabs(hvec(1)).le.1d-10) then
                     ...
                     if (hvec(1).gt.1d-10) then
                        ...
                        if (hvec(1).lt.-1d-10) then
                           ...
                           write(*,*)'Cannot find photon position-X'                    
                           stop
                        endif
                     endif
                  endif 
                  ...
                  if (dabs(hvec(2)).le.1d-10) then
                     ...
                     if (hvec(2).gt.1d-10) then
                        ...
                        if (hvec(2).lt.-1d-10) then
                           ...
                           write(*,*)'Cannot find photon position-Y'                    
                           stop
                        endif
                     endif   
                  endif     
                  ...
                  if (dabs(hvec(3)).le.1d-10) then
                     ...
                     if (hvec(3).gt.1d-10) then
                        ...
                        if (hvec(3).lt.-1d-10) then
                           ...
                           write(*,*)'Cannot find photon position-Z'                    
                           stop
                        endif
                     endif   
                  endif                               
...
c
c
               volfil=volfilar(ci(1),ci(2),ci(3))    
               vllmt=0.1  
c               
c              Whether photon reach Rin (remp) or useless cell
               if (((drvectmp.le.remp).and.(drvec.lt.remp)
     &            .and.(drvectmp.gt.rstar))
     &            .or.((drvectmp.lt.diend).and.(volfil.lt.vllmt))) then
                  cwi(1)=0.0d0
                  cwi(2)=0.0d0
                  cwi(3)=0.0d0
                  cwi(4)=0.0d0
...
c
ccc               if (abs(dsqrt((rvec(1)-strpos(1))**2.0d0
ccc     &                      +(rvec(2)-strpos(2))**2.0d0
ccc     &                      +(rvec(3)-strpos(3))**2.0d0)-0.75d18)
ccc     &                       .le.0.00001d18)
ccc     &         jnuw_arr(1,nup) = jnuw_arr(1,nup)+ds*mcde/dv 
ccc               if (abs(dsqrt((rvec(1)-strpos(1))**2.0d0
ccc     &                      +(rvec(2)-strpos(2))**2.0d0
ccc     &                      +(rvec(3)-strpos(3))**2.0d0)-1.75d18)
ccc     &                       .le.0.00001d18)               
ccc     &         jnuw_arr(2,nup) = jnuw_arr(2,nup)+ds*mcde/dv               
c
...
c                  
     ...
     &             =jnu_arr(ci(1),ci(2),ci(3),nup)+ds*mcde/(dv*volfil)
c
c
                  lgstr=.false.
                  lgdif=.true.
...
c
   ...
   10 if (rvec(1).lt.cwx(i)) then 
      ...
      if (dabs(rvec(1)-cwx(i)).le.1d-10) then 
         cwi(1)=i                          
         cwi(2)=i  
      else
         if (rvec(1).lt.cwx(i+1)) then
            ...
            if (i.lt.mcnx) then
               ...
               if ((dabs(rvec(1)-cwx(i+1)).le.1d-10)
     &            .and.((i+1).eq.(mcnx+1))) then
                  cwi(1)=i+1
                  cwi(2)=i+1
               else
...
c
   ...
   20 if (rvec(2).lt.cwy(i)) then 
      ...
      if (dabs(rvec(2)-cwy(i)).le.1d-10) then 
         cwi(3)=i                          
         cwi(4)=i  
      else
         if (rvec(2).lt.cwy(i+1)) then
            ...
            if (i.lt.mcny) then
               ...
               if ((dabs(rvec(2)-cwy(i+1)).le.1d-10)
     &            .and.((i+1).eq.(mcny+1))) then
                  cwi(3)=i+1
                  cwi(4)=i+1
               else
...
c
   ...
   30 if (rvec(3).lt.cwz(i)) then 
      ...
      if (dabs(rvec(3)-cwz(i)).le.1d-10) then 
         cwi(5)=i                          
         cwi(6)=i  
      else
         if (rvec(3).lt.cwz(i+1)) then
            ...
            if (i.lt.mcnz) then
               ...
               if ((dabs(rvec(3)-cwz(i+1)).le.1d-10)
     &            .and.((i+1).eq.(mcnz+1))) then
                  cwi(5)=i+1
                  cwi(6)=i+1
               else
